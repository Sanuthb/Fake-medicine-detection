    ...
    getData:function(event) {
        ...
        web3.eth.getAccounts(function(error,accounts){
            ...
            App.contracts.medicine.deployed().then(function(instance){
                ...
                return medicineInstance.queryPharmaciesList(web3.fromAscii(manufacturerCode),{from:account});
                ...
                for(var k=0;k<result[0].length;k++){
                    pharmacyId[k]=result[0][k];
                }

                for(var k=0;k<result[1].length;k++){
                    pharmacyName[k]=web3.toAscii(result[1][k]);
                }

                for(var k=0;k<result[2].length;k++){
                    pharmacyLicense[k]=web3.toAscii(result[2][k]);
                }

                for(var k=0;k<result[3].length;k++){
                    pharmacyCode[k]=web3.toAscii(result[3][k]);
                }

                for(var k=0;k<result[4].length;k++){
                    pharmacyPhone[k]=result[4][k];
                }

                for(var k=0;k<result[5].length;k++){
                    pharmacistName[k]=web3.toAscii(result[5][k]);
                }

                for(var k=0;k<result[6].length;k++){
                    pharmacyAddress[k]=web3.toAscii(result[6][k]);
                }
                
                var t= "";
                document.getElementById('logdata').innerHTML = t;
                for(var i=0;i<result[0].length;i++) {
                    var temptr = "<td>"+pharmacyPhone[i]+"</td>";
                    if(temptr === "<td>0</td>"){
                        break;
                    }
                    var tr="<tr>";
                    tr+="<td>"+pharmacyId[i]+"</td>";
                    tr+="<td>"+pharmacyName[i]+"</td>";
                    tr+="<td>"+pharmacyLicense[i]+"</td>";
                    tr+="<td>"+pharmacyCode[i]+"</td>";
                    tr+="<td>"+pharmacyPhone[i]+"</td>";
                    tr+="<td>"+pharmacistName[i]+"</td>";
                    tr+="<td>"+pharmacyAddress[i]+"</td>";
                    tr+="</tr>";
                    t+=tr;
                }
                document.getElementById('logdata').innerHTML += t;
